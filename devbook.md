...
#### 🔴 Le régime matrimonial était du texte libre

Signalé à l'usage. Mesuré en base : **quatre orthographes pour deux régimes** — « Communauté de
biens », « Communaté de bien » (deux fautes), « Séparation », « Séparation de biens ». Les modèles
Word substituant la valeur brute, la faute partait **telle quelle dans les actes authentiques**, et
un client était marié sous « Communaté de bien ».

Ce n'était pas un oubli de conception mais d'application : `situation_matrimoniale` était **déjà**
un `select` à 10 endroits ; seul le régime est resté en `type: 'text'` dans ses 8 déclarations.

- [x] `Client::REGIMES_MATRIMONIAUX` (4 régimes du socle civiliste) et
  `Client::SITUATIONS_MATRIMONIALES` — le serveur fait autorité. ⚠️ Je ne garantis pas que ces
  quatre régimes soient tous praticables en Guinée, ni lequel s'applique à défaut de contrat : la
  liste est à **un seul endroit**, corrigeable en une ligne
- [x] Les **8 déclarations** du régime passent en `select`, et les **10 de la situation** cessent de
  recopier leurs options — même défaut que celui déjà corrigé sur `FORMES_SOCIETE`, dont le
  commentaire disait « les deux formes ajoutées n'y sont jamais entrées ». Le `placeholder`
  « Communauté de biens / Séparation de biens » disparaît : il n'annonçait que deux valeurs sur
  quatre, ce qui explique probablement les saisies observées
- [x] **Trois règles complémentaires** sur le régime, et non redondantes : `required_if` (l'acte le
  nomme), `prohibited_unless` (un régime sans mariage décrit un état impossible) et `Rule::in`
- [x] La duplication PHP/JS est inévitable — les questionnaires sont statiques — mais **gardée** :
  un test compare les deux listes et vérifie qu'aucune déclaration ne recopie plus les options
- [x] **Reprise des 2 fiches clients** (`normaliser_regimes_matrimoniaux_clients`). Nécessaire et
  pas cosmétique : sans elle, `Rule::in` aurait rendu le client #14 **inenregistrable**, même pour
  un changement de téléphone. Les correspondances douteuses ne sont **pas devinées** — journalisées
  et laissées en place
- [x] ⚠️ **Les questionnaires ne sont pas réécrits.** Ils alimentent des actes déjà produits ; le
  `select` affiche une valeur hors liste **sélectionnée et signalée**, jamais effacée en silence —
  même motif que `LieuSelect`. Vérifié : les 4 valeurs des questionnaires sont intactes, et plus
  aucun client n'est hors liste
- [x] `ListesMatrimonialesTest` (7 tests) + 3 dans `ControlesClientTest`. **404 tests, 396 passent**
...
⚠️ **`donnees` porte les dates en `JJ/MM/AAAA`, les colonnes castées `date` en ISO** — deux formats
légitimes, dont la confusion a produit cinq défauts (voir décision #40). La projection depuis une
fiche client écrivait un horodatage brut : `${pp.date_naissance}` imprimait
`1970-02-05T00:00:00.000000Z` dans l'acte, et ni `_jma` ni `_lettres` n'étaient dérivées, faute de
reconnaître le format. Le contrat est énoncé en tête de `resources/js/lib/dates.js` et rendu
exécutable par `FormatsDatesTest` (aucune valeur de `donnees` ne doit correspondre à un motif ISO).
...
| 40 | Les dates ont **deux formats contractuels** — `JJ/MM/AAAA` dans `questionnaires.donnees`, ISO dans les colonnes castées `date` — et la conversion se fait **avant validation**, en un seul endroit | Les deux formats sont légitimes : le modèle Word reçoit la valeur de `donnees` telle quelle (et n'en dérive `_jma`/`_lettres` que depuis du français), tandis que Carbon et MySQL ne lisent que de l'ISO. Ce qui manquait n'était pas une règle mais son énoncé : cinq endroits la violaient. Deux modales postaient du français vers une colonne castée, où PHP lit du **mois/jour américain** — `13/05/1985` faisait échouer `strtotime` et la règle `date` refusait une date valide (l'erreur signalée par l'étude), tandis que `01/04/1985` devenait le **4 janvier**, enregistré sans la moindre erreur. Trois autres modales faisaient déjà le bon geste : le correctif est un alignement, pas une invention. Symétriquement, la projection écrivait de l'ISO dans `donnees` et le miroir JS y recopiait l'horodatage de l'API, imprimé tel quel dans l'acte authentique. **Pourquoi les tests ne l'ont pas vu** : `ControlesClientTest` était écrit en ISO — il éprouvait les règles, jamais le format que le formulaire envoie ; `ClientProjectionTest` allait plus loin et *verrouillait* l'ISO dans `donnees`. La leçon est portée par `FormatsDatesTest`, qui part du format de l'interface. Conversion plutôt que refus dans `Normalisation::datesEnISO()` : un import historique reste servi, et c'est le **silence** qui était grave, pas le format. Reprise des données : 3 fiches clients (7 dates) réinversées — jour ≤ 12 seulement, au-delà l'inversion était impossible — avec un témoin `dates_a_confirmer` hors `$fillable` qui affiche les **deux lectures** et invite au recontrôle sur la pièce d'identité, car le mécanisme est certain mais une ligne prise isolément reste ambiguë ; il se lève au premier enregistrement de la fiche. Les 6 clés de questionnaire en horodatage sont normalisées **de forme, jamais de fond** — ce qui la distingue de la reprise des régimes matrimoniaux (où l'on aurait corrigé la saisie d'une personne) : on ne répare ici qu'une sérialisation qui rendait l'acte inutilisable |
